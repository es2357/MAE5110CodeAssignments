...
: advances the simulation of rimless wheel through time,
handling collisions and checking for settling
(and performs sanity checks on the dynamics and impact reset)
"""
...
def find_impact_time(
    t, state, timestep, params, direction: model.ImpactDirection, *, impact_time_tol=1e-8,
):
    ...
    if direction == model.ImpactDirection.DOWNHILL:
...
def should_settle(state, params):
    ...
    and diagnose the settling outcome early-- this prevents the code
    from encountering an infinite loop, especially at small gamma values"""
...
def step(
    t, state, timestep, params, *,
    impact_time_tol=1e-8,
    remaining_time_tol=0.0,
    stop_at_first_impact=False,
):
    """Take one timestep, handling all impacts that occur during the step.

    Tolerances control impact-time refinement and the smallest remaining
    timestep to integrate. Defaults preserve this script's simulation settings.
    With stop_at_first_impact=True, return the first post-impact state before
    checking for settling; its time is impacts[0]["time"], not t + timestep.
    """
    ...
    while remaining_time > remaining_time_tol:
        ...
        if not np.all(np.isfinite(trial_state)):
            raise RuntimeError("Nonfinite state while integrating the rimless wheel.")

        direction = model.impact_direction(trial_state, params)

        # if no impact in the remaining time
        if direction == model.ImpactDirection.NONE:
        ...
        if stop_at_first_impact:
            return state_plus, impacts, False

        if len(impacts) > MAX_IMPACTS_PER_STEP:
            raise RuntimeError("Too many impacts in one timestep.")

        # Check whether the wheel has enough energy to
        # continue rolling over upright
        if direction == model.ImpactDirection.DOWNHILL and should_settle(state_plus, params):
            state_plus[1] = 0.0
...
def main():
    """Run the rimless-wheel sanity checks, example simulation, and plots."""
    ...
    assert model.impact_direction(state_plus, params) == model.ImpactDirection.NONE
    ...
    for k, t in enumerate(time_traj):
        ...
        while (
            ...
            current_contact_height -= (
                direction.value * 2 * length * np.sin(alpha) * np.sin(gamma)
            )
